...
export async function playCue(url, { onEnded } = {}) {
  ...
  a.addEventListener('error', () => {
    if (audio !== a) return
    console.error('[cuePlayer] audio element error',
      { src: a.src, code: a.error?.code, msg: a.error?.message })
    const cb = endedCb
    audio   = null
    endedCb = null
  ...
  try {
    ...
    if (audio === a) {
      ...
      audio   = null
      endedCb = null
      console.error('[cuePlayer] play() rejected:', err?.name, err?.message ?? err)
      if (typeof cb === 'function') { try { cb() } catch {} }
    }
  }
}
